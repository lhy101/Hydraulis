...
from hydraulis.nn.modules.parallel_multi_ds import parallel_data_provider, parallel_multi_data_provider, get_multi_ds_parallel_config
...
# self-attn
class LLamaAttention(hydraulis.nn.Module):
    ...
    def forward(
        ...
        def apply_rotary_pos_emb(x, _name='q'):
            ...
            # 去除zero
            ds_hierarchy = [
                hydraulis.DistributedStatesUnion([hydraulis.DistributedStates(ds.device_num, {-1: ds.device_num}, [-1]) for ds in ds_union.ds_list], ds_union.hetero_dim)
                    for ds_union in ds_hierarchy
            ]
            sin_global = hydraulis.from_numpy_parallel(sin_np, ds_hierarchy, device_group_hierarchy=device_group_hierarchy, requires_grad=False, name=f'sin_{_name}')
            cos_global = hydraulis.from_numpy_parallel(cos_np, ds_hierarchy, device_group_hierarchy=device_group_hierarchy, requires_grad=False, name=f'cos_{_name}')
            out = hydraulis.rotary(x, cos_global, sin_global, inplace=True)
            return out
        ...
        attn_output = hydraulis.parallel_attn(
            qkv,             
            self.head_dim, 
...
class ParallelMLP(hydraulis.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        intermediate_parallel = hydraulis.swiglu(intermediate_parallel)
...
class LLamaMLP(hydraulis.nn.Module):
    def __init__(self, config, ds_parallel_configs, layer_idx, name='mlp'):
...
class LLamaBlock(hydraulis.nn.Module):
    def __init__(self, config, ds_parallel_configs, layer_idx):
        ...
        self.rmsnorm_1 = hydraulis.nn.HtMultiParallelLayerNorm(hidden_size, get_multi_ds_parallel_config(ds_parallel_configs, 'layernorm1', layer_idx), sp=True, name=f'rmsnorm1_block{layer_idx}')
        ...
        self.rmsnorm_2 = hydraulis.nn.HtMultiParallelLayerNorm(hidden_size, get_multi_ds_parallel_config(ds_parallel_configs, 'layernorm2', layer_idx), sp=True, name=f'rmsnorm2_block{layer_idx}')
...
class LLamaModel(hydraulis.nn.Module):
    def __init__(self, config, ds_parallel_configs):
        ...
        self.rmsnorm_f = hydraulis.nn.HtMultiParallelLayerNorm(self.embed_dim, get_multi_ds_parallel_config(ds_parallel_configs, 'layernorm_final'), sp=True, name='rmsnorm_final')
        ...
        if sp:
            ...
            for ds_union_input in ds_hierarchy_input:
                ...
                for ds_input in ds_union_input.ds_list:
                    ds_split0 = hydraulis.DistributedStates(ds_input.device_num, {0: ds_input.device_num}, [0])
                ...
                ds_hierarchy_output.append(hydraulis.DistributedStatesUnion(ds_list_split0, 0 if ds_union_input.hetero_dim != -3 else -3))
            # [b * seq_len // tp, embed_dim]
            hidden_states = hydraulis.comm(hidden_states, ds_hierarchy_output, name="workaround_sp_scatter")
        ...
        for i, block in enumerate(self.h):
            ...
            if i != len(self.h) - 1:
                ...
                if next_block.rmsnorm_1.sp:
                    hidden_states = hydraulis.comm(hidden_states, next_block.rmsnorm_1.ds_union_map['split0'], next_block.rmsnorm_1.device_group_unions, name=f"pipeline_layer_{i}_comm")
                else:
                    hidden_states = hydraulis.comm(hidden_states, next_block.attn.qkv_dense.ds_union_map['split0_dup'], next_block.rmsnorm_1.device_group_unions, name=f"pipeline_layer_{i}_comm")
        # layernorm
        hidden_states = self.rmsnorm_f(hidden_states)
        return hidden_states

class LLamaLMHeadModel(hydraulis.nn.Module):
    ...
    def forward(
        ...
        if not hidden_states.check_ds_hierarchy_equal(self.lm_head.ds_union_map['split0_dup']):
            hidden_states = hydraulis.comm(hidden_states, self.lm_head.ds_union_map['split0_dup'])
        '''
        ...
        if labels is not None:
            loss = hydraulis.vocab_parallel_cross_entropy(lm_logits,
