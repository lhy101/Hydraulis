...
from hydraulis.nn.modules.parallel_multi_ds import config2ds
...
def kill_command():
    # 定义包含所有主机的文件
    hosts_file = '/home/pkuhydraulis/lhy/multi_switch/examples/nlp/gpt/scripts/host.txt'
    # 定义要查找并杀掉的进程的关键字
    process_keyword = 'python3 -u train_hydraulis_train.py'
    # 使用 pssh 执行远程命令
...
def pretrain(args):
    ...
    for i in range(len(input_ds_hierarchy)):
        ...
        # 表示各个dcp分到的seq len
        config.multi_seq_lens_symbol.append([hydraulis.IntSymbol(1) for _ in range(dcp_size)])
        # 例如[0, 0, 0, 1, 1, 2, 2, 2] 
        # 相同编号的在一个cp group中
        config.multi_cp_group_symbol.append([hydraulis.IntSymbol(1) for _ in range(dcp_size)])
        
    # todo: remove the global_shape
    # now just offer a shape that can be divided by dcp size
    input_ids = hydraulis.parallel_placeholder(hydraulis.int64, global_shape=[dcp_size], ds_hierarchy=input_ds_hierarchy, device_group_hierarchy=input_dg_hierarchy, name='input_ids')
    # position_ids = hydraulis.parallel_placeholder(hydraulis.int64, global_shape=[dcp_size], ds_hierarchy=input_ds_hierarchy, device_group_hierarchy=input_dg_hierarchy, name='position_ids')
    # token_type_ids = hydraulis.parallel_placeholder(hydraulis.int64, global_shape=[dcp_size], ds_hierarchy=input_ds_hierarchy, device_group_hierarchy=input_dg_hierarchy, name='token_type_ids')
    # attention_mask = hydraulis.parallel_placeholder(hydraulis.float32, global_shape=[dcp_size], ds_hierarchy=input_ds_hierarchy, device_group_hierarchy=input_dg_hierarchy, name='attention_mask')
    masked_lm_labels = hydraulis.parallel_placeholder(hydraulis.int64, global_shape=[dcp_size], ds_hierarchy=label_ds_hierarchy, device_group_hierarchy=label_dg_hierarchy, name='masked_lm_labels')
    ...
    def run_plan(
        ...
        for step in range(steps):
            ...
            start_time = time.time()
            if step == 2:
                os.environ['HYDRAULIS_STRAGGLER'] = "EXP_NEW"
                os.environ['HYDRAULIS_STRAGGLER_LOG_FILE'] = args.exp_file
            try:
    ...
    def test_single_round(): 
        ...
        consumed_samples = run_plan(
            ...
            run_level = hydraulis.run_level("compute_only")
        )
...
if __name__ == '__main__':
    ...
    with hydraulis.graph("define_and_run", num_strategy=args.num_strategy):
        if args.bf16:
