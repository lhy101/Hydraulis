...
class ConvNd(Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: Tuple[int, ...],
        ...
        with hydraulis.graph("define_and_run"):
            ...
            self.weight = hydraulis.nn.functional.kaiming_uniform_([out_channels, in_channels, *kernel_size], 
            ...
            if bias:
                ...
                self.bias = hydraulis.rand([out_channels], -bound, bound, requires_grad=True)
            else:
                self.bias = None
    ...
    def reset_parameters(self) -> None:
        self.weight = hydraulis.nn.init.kaiming_uniform_(a=math.sqrt(5))
...
class Conv2d(ConvNd):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.bias is None:
            return hydraulis.conv2d(input, self.weight, self.padding[0], self.stride[0])
        else:
            return hydraulis.conv2d(input, self.weight, self.bias, self.padding[0], self.stride[0])
        
    def to(self, dtype):
        with hydraulis.graph("define_and_run"):
            self.weight = self.weighydraulis.to(datatype = dtype)
            if (self.bias is not None):
