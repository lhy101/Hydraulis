...
#include "hydraulis/_binding/utils/python_primitives.h"
#include "hydraulis/_binding/utils/numpy.h"
#include "hydraulis/utils/string_utils.h"
...
std::string ArgType2Str(ArgType type) {
  switch (type) {
    ...
    case ArgType::DATA_TYPE:
      return "hydraulis.dtype";
    case ArgType::DEVICE:
      return "hydraulis.device";
    case ArgType::DEVICE_GROUP:
      return "hydraulis.DeviceGroup";
    case ArgType::DEVICE_GROUP_LIST:
      return "List[hydraulis.DeviceGroup]";
    case ArgType::DG_HIERARCHY:
      return "List[List[hydraulis.DeviceGroup]]";
    case ArgType::STREAM:
      return "hydraulis.stream";
    case ArgType::ND_ARRAY:
      return "hydraulis.NDArray";
    case ArgType::ND_ARRAY_LIST:
      return "List[hydraulis.NDArray]";
    case ArgType::TENSOR:
      return "hydraulis.Tensor";
    case ArgType::TENSOR_LIST:
      return "List[hydraulis.Tensor]";
    case ArgType::OPERATOR:
      return "hydraulis.Operator";
    case ArgType::OPERATOR_LIST:
      return "List[hydraulis.Operator]";
    ...
    case ArgType::DISTRIBUTED_STATES:
      return "hydraulis.DistributedStates";
    case ArgType::DISTRIBUTED_STATES_LIST:
      return "List[hydraulis.DistributedStates]";
    case ArgType::DS_HIERARCHY:
      return "List[hydraulis.DistributedStatesUnion]";
    case ArgType::INT_SYMBOL:
      return "hydraulis.IntSymbol";
    case ArgType::SYMBOLIC_SHAPE:
      return "List[hydraulis.IntSymbol]";
    case ArgType::SYMBOLIC_SHAPE_LIST:
      return "List[List[hydraulis.IntSymbol]]";
    case ArgType::INITIALIZER:
      return "hydraulis.Initializer";
    default:
...
ArgType Str2ArgType(const std::string& type) {
  ...
  if (type == "hydraulis.dtype" || type == "dtype" || type == "DataType")
    return ArgType::DATA_TYPE;
  if (type == "hydraulis.device" || type == "device" || type == "Device") 
    return ArgType::DEVICE;
  if (type == "hydraulis.DeviceGroup" || type == "DeviceGroup")
    return ArgType::DEVICE_GROUP;
  if (type == "List[hydraulis.DeviceGroup]" || type == "List[DeviceGroup]" ||
      type == "hydraulis.DeviceGroupList" || type == "DeviceGroupList")
    return ArgType::DEVICE_GROUP_LIST;
  if (type == "List[List[hydraulis.DeviceGroup]]" || type == "List[List[DeviceGroup]]" || 
      type == "DeviceGroupHierarchy")
    return ArgType::DG_HIERARCHY;
  if (type == "hydraulis.stream" || type == "stream" || type == "Stream") 
    return ArgType::STREAM;
  if (type == "hydraulis.NDArray" || type == "NDArray" || 
      type == "hydraulis.ndarray" || type == "ndarray") 
    return ArgType::ND_ARRAY;
  if (type == "List[hydraulis.NDArray]" || type == "List[NDArray]" || 
      type == "List[hydraulis.ndarray]" || type == "List[ndarray]" || 
      type == "NDArrayList") 
    return ArgType::ND_ARRAY_LIST;
  if (type == "hydraulis.Tensor" || type == "Tensor" || 
      type == "hydraulis.tensor" || type == "tensor") 
    return ArgType::TENSOR;
  if (type == "List[hydraulis.Tensor]" || type == "List[Tensor]" ||
      type == "List[hydraulis.tensor]" || type == "List[tensor]" || 
      type == "TensorList") 
    return ArgType::TENSOR_LIST;
  if (type == "hydraulis.Operator" || type == "Operator" || 
      type == "hydraulis.operator" || type == "operator")
      return ArgType::OPERATOR;
  if (type == "List[hydraulis.Operator]" || type == "List[Operator]" ||
      type == "List[hydraulis.operator]" || type == "List[operator]" || 
  ...
  if (type == "hydraulis.DistributedStates" || type == "DistributedStates")
    return ArgType::DISTRIBUTED_STATES;
  if (type == "List[hydraulis.DistributedStates]" || type == "DistributedStatesList" ||
      type == "List[DistributedStates]")
    return ArgType::DISTRIBUTED_STATES_LIST; 
  if (type == "List[hydraulis.DistributedStatesUnion]" || type == "List[DistributedStatesUnion]" ||
      type == "DistributedStatesHierarchy")
    return ArgType::DS_HIERARCHY;
  if (type == "hydraulis.IntSymbol" || type == "IntSymbol")
    return ArgType::INT_SYMBOL;
  if (type == "List[hydraulis.IntSymbol]" || type == "List[IntSymbol]" || type == "SyShape")
    return ArgType::SYMBOLIC_SHAPE;
  if (type == "List[List[hydraulis.IntSymbol]]" || type == "List[List[IntSymbol]]" || type == "List[SyShape]")
    return ArgType::SYMBOLIC_SHAPE_LIST;
  if (type == "hydraulis.Initializer" || type == "Initializer")
