...
//       << "Type " << cls_type->tp_name << " is not derived from hydraulis.Dataloader";
...
//       << "Subclass of hydraulis.Dataloader must be created from a variable. "
...
std::vector<PyMethodDef> InitDataloaderPyMethodDefs() {
  ...
  AddPyMethodDefs(ret, hydraulis::graph::get_registered_dataloader_methods());
  return ret;
}
...
std::vector<PyMethodDef> InitDataloaderPyClassMethodDefs() {
  ...
  AddPyMethodDefs(ret, hydraulis::graph::get_registered_dataloader_class_methods());
  return ret;
}
