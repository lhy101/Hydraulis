...
#include "hydraulis/_binding/graph/tensor_ctor.h"
...
//       << "Type " << cls_type->tp_name << " is not derived from hydraulis.SGDOptimizer";
...
//       << "Subclass of hydraulis.SGDOptimizer must be created from a variable. "
...
PyTypeObject PySGDOptimizer_Type_obj = {
  ...
  "hydraulis.SGDOptimizer", /* tp_name */
...
std::vector<PyMethodDef> InitSGDOptimizerPyMethodDefs() {
  ...
  AddPyMethodDefs(ret, hydraulis::graph::get_registered_optimizer_methods());
  return ret;
}
...
std::vector<PyMethodDef> InitSGDOptimizerPyClassMethodDefs() {
  ...
  AddPyMethodDefs(ret, hydraulis::graph::get_registered_optimizer_class_methods());
  return ret;
}
