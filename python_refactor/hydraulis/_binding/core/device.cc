...
PyObject* PyDevice_reduce(PyDevice* self) {
  ...
  py::object hydraulis_module = py::module::import("hydraulis");
  py::object hydraulis_device = hydraulis_module.attr("device");
  PyTuple_SET_ITEM(ret, 0, hydraulis_device.release().ptr());
...
PyObject* PyDeviceGroup_reduce(PyDeviceGroup* self) {
  ...
  py::object hydraulis_module = py::module::import("hydraulis");
  py::object hydraulis_device_group = hydraulis_module.attr("DeviceGroup");
  PyTuple_SET_ITEM(ret, 0, hydraulis_device_group.release().ptr());
...
ContextManager<hydraulis::graph::DeviceGroupHierarchy>& get_dg_hierarchy_ctx() {
  static ContextManager<hydraulis::graph::DeviceGroupHierarchy> dg_hierarchy_ctx;
