# TODO: output the library to `hydraulis` directory
import _hydraulis_core
from _hydraulis_core import *

from typing import Union, Tuple, List, Dict, Set, Any, Callable, Iterator, Optional, TypeVar
_tensor_type_t = TypeVar('T', bound='Tensor')

from hydraulis import nn as nn
from hydraulis import utils as utils
# from hydraulis import optim as optim

import builtins # bool is resovled as hydraulis.bool
...
class _OpContext(object):
    ...
    def __enter__(self):
        _hydraulis_core._internal_context.push_op_ctx(
    ...
    def __exit__(self, e_type, e_value, e_trace):
        _hydraulis_core._internal_context.pop_op_ctx(
...
class _GraphContext(object):
    def __init__(self, g, create_new=False, prefix="default", num_strategy=-1, tmp=False):
        ...
        if isinstance(g, Graph):
            assert create_new == False, f"Using type {type(g).__name__} to create a hydraulis.Graph is not allowed, please use a str instead."
            self.graph = g
        elif isinstance(g, builtins.str):
            if create_new:
                if g == "define_and_run":
                    self.graph = _hydraulis_core._internal_context.make_new_define_and_run_graph(prefix + '_' + g)
                elif g == "eager":
                    self.graph = _hydraulis_core._internal_context.make_new_eager_graph(prefix + '_' + g)
                else:
                    raise NotImplementedError("Can only create define_and_run or eager graph for now.")
            else:
                if prefix == "default":
                    if g == "eager":
                        self.graph = _hydraulis_core._internal_context.get_default_eager_graph()
                    elif g == "define_and_run":
                        self.graph = _hydraulis_core._internal_context.get_default_define_and_run_graph()
                    elif g == "define_by_run":
                        self.graph = _hydraulis_core._internal_context.get_default_define_by_run_graph()
                else:
                    self.graph = _hydraulis_core._internal_context.get_graph(prefix + '_' + g)
        elif isinstance(g, builtins.int):
            assert create_new == False, f"Using type {type(g).__name__} to create a hydraulis.Graph is not allowed, please use a str instead."
            self.graph = _hydraulis_core._internal_context.get_graph(g)
        else:
            raise ValueError(f"Cannot parse type '{type(g).__name__}' as hydraulis.Graph")
        if num_strategy >= 1:
    ...
    def __enter__(self):
        ...
        _hydraulis_core._internal_context.push_graph_ctx(self.graph.id)
    ...
    def __exit__(self, e_type, e_value, e_trace):
        if self.wrapped_context:
            return
        _hydraulis_core._internal_context.pop_graph_ctx()
        cur_graph_contexts.remove(self)
        if self.tmp:
            graph_id = self.graph.id
            self.graph = None
            _hydraulis_core._internal_context.delete_graph(graph_id)
...
class _AutocastContext(object):
    def __init__(self, dtype = None):
        if dtype is None:
            self.autocast = _hydraulis_core._internal_context.get_default_autocast()
        else:
            self.autocast = _hydraulis_core._internal_context.make_new_autocast(True, dtype)

    def __enter__(self):
        _hydraulis_core._internal_context.push_autocast_ctx(self.autocast.id)
        return self
    
    def __exit__(self, e_type, e_value, e_trace):
        _hydraulis_core._internal_context.pop_autocast_ctx()
...
class _RunLevel(object):
    def __init__(self, type):
        if type == "update":
            ...
            raise ValueError(f"No level for '{type}' in hydraulis.Graph")
...
class _MergeStrategyContext(object):
    def __init__(self, target_graph="default", num_strategy=-1):
        self.graph = _hydraulis_core._internal_context.make_new_define_and_run_graph("tmp_define_and_run")
        self.graph.set_num_strategy(num_strategy)
        if target_graph == "default":
            self.target_graph = _hydraulis_core._internal_context.get_default_define_and_run_graph()
        else:
            self.target_graph = _hydraulis_core._internal_context.get_graph(target_graph + '_' + "define_and_run")
...
class _RecomputeContext(object):
    ...
    def __enter__(self):
        ...
        _hydraulis_core._internal_context.push_recompute_ctx(cur_multi_recompute)
        return self

    def __exit__(self, e_type, e_value, e_trace):
        _hydraulis_core._internal_context.pop_recompute_ctx()
...
class _CPUOffloadContext(object):
    def __enter__(self):
        _hydraulis_core._internal_context.push_cpu_offload_ctx()
        return self
    
    def __exit__(self, e_type, e_value, e_trace):
        _hydraulis_core._internal_context.pop_cpu_offload_ctx()
...
class _ProfileContex(object):
    def __init__(self, enabled : bool = True, use_cpu : bool = False, use_cuda : bool = False,
      ...
      self.profile = _hydraulis_core._internal_context.make_new_profile(enabled, use_cpu, use_cuda, record_shapes, profile_memory)

    def __enter__(self):
        _hydraulis_core._internal_context.push_profile_ctx(self.profile.id)
        return self.profile

    def __exit__(self, e_type, e_value, e_trace):
        _hydraulis_core._internal_context.pop_profile_ctx()
...
class _SubGraphContext(object):
    def __init__(self, name = "global", module_type = ""):
        if name is None:
            self.subgraph = _hydraulis_core._internal_context.get_default_subgraph()
        else:
            self.subgraph = _hydraulis_core._internal_context.make_new_subgraph(name=name, use_relative=True, module_type=module_type)
    def __enter__(self):
        _hydraulis_core._internal_context.push_subgraph_ctx(self.subgraph.global_name)
        return self
    
    def __exit__(self, e_type, e_value, e_trace):
        _hydraulis_core._internal_context.pop_subgraph_ctx()
...
def add_to_subgraph(tensor, global_name=""):
    _hydraulis_core._internal_context.add_op_to_subgraph(tensor, global_name)
