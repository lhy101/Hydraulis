...
# from hydraulisrpc_polling_server import server_launch
from hydraulisrpc_async_server import server_launch
...
def pssh(args):
    ...
    for i, hostname in enumerate(hostnames):
        ...
        # log编号是进程编号
        # 但不能保证分配到同样编号的rank
        cmd_list.append(cmd + f"export HYDRAULIS_LOCAL_HOSTNAME={hostname} && " + train_command + f" 2>&1 | tee {args.log_path}" + "/log_" + f"{i}" + ".txt")
    clients = []
    outputs = []
