...
class Optimizer(object):
    ...
    def step(self):
        ...
        hydraulis.group(update_ops).get_or_compute()
    ...
    def minimize(self, loss, var_list=None, name=None, grad_loss=None):
        ...
        for g, v in grads_and_vars:
            ...
            update_ops.append(update_op)
        return hydraulis.group(update_ops, name=name)
    
    def compute_gradients(self, loss, var_list=None, grad_loss=None):
        if var_list is None:
            var_list = hydraulis.requires_grad_variables(loss)
        else:
            for v in var_list:
                assert v.requires_grad, f"Variable {v} is not requires_grad"
        # for v in var_list:
        #     print(v, " ", v.requires_grad)
        grad_list = hydraulis.gradients(loss, var_list, grad_loss)
