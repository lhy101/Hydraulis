...
class Straggler(Args):
    ...
    def begin_profile(self):
        ...
        os.environ['HYDRAULIS_STRAGGLER_LOG_FILE'] = self.log_file
        
    def end_profile(self):
        assert os.path.exists(self.log_file) and "HYDRAULIS_STRAGGLER_LOG_FILE" in os.environ, \
    ...
    def run_profile(self):
        with hydraulis.graph("eager", create_new=True, prefix="tmp", tmp=True):
            with hydraulis.context(eager_device=self.local_device):
    ...
    def run_workload(self):
        ...
        x = hydraulis.from_numpy(x_np)
        y = hydraulis.from_numpy(y_np)
        z = hydraulis.from_numpy(z_np)
        x = hydraulis.matmul(hydraulis.matmul(x, y), z)
