...
#include <cstdint>
#include <iomanip>
#include "hydraulis/common/collection_streaming.h"
 ...
 public:
  ...
  : _os(os), _verbose(verbose), _file(file), _line(line) {
    static_assert(std::is_base_of<std::exception, Exception>::value,
                  "Exception must be derived from hydraulis_exception");
    if (_verbose) {
      auto now = std::chrono::system_clock::now();
      auto time = std::chrono::system_clock::to_time_t(now);
...
#define __HT_LOG_TRACE                                                         \
  hydraulis::logging::Logger(__FILENAME__, __LINE__,                                \
                        hydraulis::logging::LOG_LEVEL::TRACE, std::cout)
#define __HT_LOG_DEBUG                                                         \
  hydraulis::logging::Logger(__FILENAME__, __LINE__,                                \
                        hydraulis::logging::LOG_LEVEL::DEBUG, std::cout)
#define __HT_LOG_INFO                                                          \
  hydraulis::logging::Logger(__FILENAME__, __LINE__,                                \
                        hydraulis::logging::LOG_LEVEL::INFO, std::cout)
#define __HT_LOG_WARN                                                          \
  hydraulis::logging::Logger(__FILENAME__, __LINE__,                                \
                        hydraulis::logging::LOG_LEVEL::WARN, std::cout)
#define __HT_LOG_ERROR                                                         \
  hydraulis::logging::Logger(__FILENAME__, __LINE__,                                \
                        hydraulis::logging::LOG_LEVEL::ERROR, std::cerr)

#define HT_LOG(severity)                                                       \
  if (log_level_enabled(hydraulis::logging::LOG_LEVEL::severity))                   \
  __HT_LOG_##severity.stream()
#define HT_LOG_IF(severity, cond)                                              \
  if (log_level_enabled(hydraulis::logging::LOG_LEVEL::severity) && (cond))         \
  __HT_LOG_##severity.stream()
...
#define __HT_FATAL_VERBOSE(Exception)                                          \
  hydraulis::logging::FatalLogger<Exception>(__FILENAME__, __LINE__, true).stream()
#define HT_FATAL_VERBOSE __HT_FATAL_VERBOSE(hydraulis::hydraulis_exception)
#define __HT_FATAL_SILENT(Exception)                                           \
  hydraulis::logging::FatalLogger<Exception>(__FILENAME__, __LINE__, false).stream()
#define HT_FATAL_SILENT __HT_FATAL_SILENT(hydraulis::hydraulis_exception)
