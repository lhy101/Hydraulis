...
class CUDAProfiler {
  public:
    ...
    : _device(device) {
      _mempool = std::dynamic_pointer_cast<hydraulis::impl::CUDACachingMemoryPool>(GetMemoryPool(device));  
    }
  ...
  protected:
    ...
    // profile memory
    std::shared_ptr<hydraulis::impl::CUDACachingMemoryPool> _mempool;
