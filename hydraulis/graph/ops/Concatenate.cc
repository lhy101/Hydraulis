...
void ConcatenateOpImpl::DoCompute(Operator& op,
  ...
  if (op->instantiation_ctx().placement.is_cpu()) {
    HT_DISPATCH_KERNEL_CPU_ONLY(op->instantiation_ctx().placement.type(), type(),
                                hydraulis::impl::Concatenate, inputs,
    ...
    if (axis == 0) {
      for (int i = 0; i < num; ++i) {
        ...
        if (!inputs.at(i)->is_equal(output)) {
          HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                                       hydraulis::impl::DataTransfer, inputs.at(i), output,
      ...
      for (int i = 0; i < num; ++i) {
        HT_DISPATCH_KERNEL_CUDA_ONLY(op->instantiation_ctx().placement.type(), type(),
                                      hydraulis::impl::Concatenate, inputs.at(i),
...
void ConcatenateGradientOpImpl::DoCompute(Operator& op,
  ...
  HT_DISPATCH_KERNEL_CPU_AND_CUDA(
    op->instantiation_ctx().placement.type(), type(), hydraulis::impl::ConcatenateGradient, inputs.at(2),
