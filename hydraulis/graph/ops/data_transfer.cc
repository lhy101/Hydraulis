...
bool DataH2DOpImpl::DoInstantiate(Operator& op, const Device& placement,
  ...
  for (size_t i = 0; i < HT_MAX_NUM_MICRO_BATCHES; i++) {
    inst_ctx.start[i] = std::make_unique<hydraulis::impl::CUDAEvent>(inst_ctx.placement);
    inst_ctx.stop[i] = std::make_unique<hydraulis::impl::CUDAEvent>(inst_ctx.placement);
  }
...
bool DataD2HOpImpl::DoInstantiate(Operator& op, const Device& placement,
  ...
  inst_ctx.stream_index = stream_id;
  for (size_t i = 0; i < HT_MAX_NUM_MICRO_BATCHES; i++) {
    inst_ctx.start[i] = std::make_unique<hydraulis::impl::CUDAEvent>(op->input(0)->placement());
    inst_ctx.stop[i] = std::make_unique<hydraulis::impl::CUDAEvent>(op->input(0)->placement());
  }
  op->output(0)->set_placement(placement);
  return true;
}
