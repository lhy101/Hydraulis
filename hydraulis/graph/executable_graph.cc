...
#include "hydraulis/graph/ops/op_headers.h"
...
#include "hydraulis/core/ndarray_storage.h"
#include <nccl.h>
#include <ctime>
#include <iostream>
#include <fstream>
...
Operator& ExecutableGraph::MakeOpInner(std::shared_ptr<OpInterface> body,
  ...
  if (op_meta.device_group_hierarchy.size() != 0) {
    ...
    auto inferred = hydraulis::impl::comm::GetLocalDevice();
      ...
      for (size_t i = 0; i < op->num_inputs(); i++) {
        ...
        if (!input->is_contiguous()) {
          ...
          if (op_id.has_value() &&
            ...
            HT_LOG_TRACE << hydraulis::impl::comm::GetLocalDevice() << ": Make Contiguous op for tensor " << input->name()
      ...
      if ((comm_type & BATCHED_ISEND_IRECV_OP) != 0) {
        ...
        if (_bridge_single_communicator) {
          for (const auto rank : _used_ranks) {
            comm_set.insert(hydraulis::impl::comm::WorldRankToDevice(rank));
          }
        }
...
void ExecutableGraph::GetExecEnvs() {
  char* env = std::getenv("HYDRAULIS_P2P");
  if (env != nullptr) {
    if (std::string(env) == "SINGLE_COMMUNICATOR") {
      ...
      hydraulis::impl::comm::NCCLCommunicationGroup::GetOrCreate(_used_ranks, Stream(hydraulis::impl::comm::GetLocalDevice(), kP2PStream));
    } else {
      HT_RUNTIME_ERROR << "Unknown hydraulis p2p setting: " + std::string(env);
    }
  } else {
  ...
  env = std::getenv("HYDRAULIS_BRIDGE");
  if (env != nullptr) {
    if (std::string(env) == "SINGLE_COMMUNICATOR") {
      _bridge_single_communicator = true;
      // 这里直接提前把communicator给create好
      // HT_LOG_INFO << "get or create single communicator for BRIDGE ops";
      hydraulis::impl::comm::NCCLCommunicationGroup::GetOrCreate(_used_ranks, Stream(hydraulis::impl::comm::GetLocalDevice(), kBridgeStream));
    } else {
      HT_RUNTIME_ERROR << "Unknown hydraulis p2p setting: " + std::string(env);
    }
  } else {
  ...
  env = std::getenv("HYDRAULIS_OVERLAP_GRAD_REDUCE");
  ...
  if (env != nullptr) {
    if (std::string(env) == "FIRST_STAGE") {
      ...
      HT_RUNTIME_ERROR << "Unknown hydraulis dp overlap setting: " + std::string(env);
    }
  }

  env = std::getenv("HYDRAULIS_SHAPE_MISMATCH");
  if (env != nullptr) {
    if (std::string(env) == "NO_MISMATCH") {
      ...
      HT_RUNTIME_ERROR << "Unknown hydraulis shape mismatch setting: " + std::string(env);
    }
  } else {
    // 默认没有shape mismatch
    _shape_mismatch_flag = 0;
  ...
  env = std::getenv("HYDRAULIS_STRAGGLER");
  if (env != nullptr) {
    if (std::string(env) == "ANALYSIS") {
      ...
      HT_RUNTIME_ERROR << "Unknown hydraulis straggler level: " + std::string(env);
    }
  } else {
    // 默认不分析straggler
    _straggler_flag = 0;
  ...
  env = std::getenv("HYDRAULIS_STRAGGLER_LOG_FILE");
  if (env != nullptr) {
  ...
  env = std::getenv("HYDRAULIS_MEMORY_PROFILE");
  if (env != nullptr) {
    if (std::string(env) == "MICRO_BATCH") {
      ...
      HT_RUNTIME_ERROR << "Unknown hydraulis memory profile level: " + std::string(env);
    }
  } else {
    // 默认不profile
    _memory_profile_level = MEMORY_PROFILE_LEVEL::WARN;
  ...
  env = std::getenv("HYDRAULIS_MEMORY_LOG_FILE");
  if (env != nullptr) {
  ...
  env = std::getenv("HYDRAULIS_PARALLEL_ATTN");
  if (env != nullptr) {
    if (std::string(env) == "ANALYSIS") {
      ...
      HT_RUNTIME_ERROR << "Unknown hydraulis parallel attn level: " + std::string(env);
    }
  } else {
    // 默认不分析parallel attn
  ...
  env = std::getenv("HYDRAULIS_PARALLEL_ATTN_LOG_FILE");
  if (env != nullptr) {
...
NDArrayList ExecutableGraph::Run(const Tensor& loss, const TensorList& fetches, 
  ...
  if (_used_ranks.size() >= 2) {
    auto& comm_group = hydraulis::impl::comm::NCCLCommunicationGroup::GetOrCreate(_used_ranks, local_device);
    comm_group->Barrier(true);
  }
  // sync partially
  /*
  std::vector<int> ranks;
  for (const auto& stage : _pipeline_map[hydraulis::impl::comm::GetLocalDevice()]) {
    for (const auto& device : stage.devices()) {
      auto rank = hydraulis::impl::comm::DeviceToWorldRank(device);
  ...
  if (ranks.size() >= 2) {
    ...
    // hydraulis::impl::comm::Barrier(ranks);
    auto& comm_group = hydraulis::impl::comm::NCCLCommunicationGroup::GetOrCreate(ranks, local_device);
    comm_group->Barrier(true);
  }
  */
  ...
  /*
  TIK(free_mempool);
  hydraulis::impl::ProfileAfterEmptyAllCUDACache(local_device);
  TOK(free_mempool);
  ...
  */
  ...
  auto profiler_optional = hydraulis::impl::Profile::get_cur_profile();
  ...
  if (_memory_profile_level == MEMORY_PROFILE_LEVEL::MICRO_BATCH && _memory_log_file_path != "") {
    std::ofstream file;
    std::string suffix = "_" + std::to_string(hydraulis::impl::comm::GetWorldRank()) + ".txt";
    ...
    if (_straggler_flag) {
      ...
      if (_straggler_log_file_path != "") {
        if (_straggler_flag == 1) {
          ...
          ofstream_sync file(_straggler_log_file_path + "_" + std::to_string(hydraulis::impl::comm::GetWorldRank()) + ".txt", std::ios_base::app);
          if (file.is_open()) {
          ...
          if (hydraulis::impl::comm::GetWorldRank() == 0) {
            ofstream_sync file(_straggler_log_file_path, std::ios_base::app);
            if (file.is_open()) {
