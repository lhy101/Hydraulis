...
namespace std {
template <>
struct hash<hydraulis::DataType> {
  std::size_t operator()(hydraulis::DataType data_type) const noexcept {
    return std::hash<int>()(static_cast<int>(data_type));
  }
};

inline std::string to_string(hydraulis::DataType data_type) {
  return hydraulis::DataType2Str(data_type);
}
} // namespace std
