...
namespace std {
template <>
struct hash<hydraulis::Stream> {
  std::size_t operator()(const hydraulis::Stream& stream) const noexcept {
    // Following boost::hash_combine
    auto hash = std::hash<hydraulis::StreamIndex>()(stream.stream_index());
    hash ^= (std::hash<hydraulis::Device>()(stream.device()) + 0x9e3779b9 +
             (hash << 6) + (hash >> 2));
    return hash;
  }
};

inline std::string to_string(const hydraulis::Stream& stream) {
