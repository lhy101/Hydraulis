...
class Device {
 public:
  ...
  static inline std::string GetLocalHostname() {
    char* env = std::getenv("HYDRAULIS_LOCAL_HOSTNAME");
    if (env == nullptr) {
      return "";
    } else {
      std::string ret(env);
...
template <>
struct hash<hydraulis::Device> {
  std::size_t operator()(const hydraulis::Device& device) const noexcept {
...
template <>
struct hash<std::pair<hydraulis::Device, hydraulis::Device>> {
  std::size_t operator()(const std::pair<hydraulis::Device, hydraulis::Device>& device_pair) const noexcept {
    // devices in pair need to sort
    auto hash_op = std::hash<hydraulis::Device>();
    size_t seed = 2;
...
template <>
struct hash<hydraulis::DeviceGroup> {
  std::size_t operator()(const hydraulis::DeviceGroup& group) const noexcept {
...
inline std::string to_string(const hydraulis::Device& device) {
  std::ostringstream os;
  os << device;
  return os.str();
}

inline std::string to_string(const hydraulis::DeviceGroup& group) {
  std::ostringstream os;
  os << group;
  return os.str();
}
