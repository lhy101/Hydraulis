...
void RollCuda(const NDArray& input, const HTShape& shift, const HTAxes& axis,
  ...
  auto strides_arr = hydraulis::cuda::to_int64_ndarray(strides, device_id);
  auto sizes_arr = hydraulis::cuda::to_int64_ndarray(sizes, device_id);
  auto shifts_arr = hydraulis::cuda::to_int64_ndarray(shifts, device_id);
  dim3 blocks, threads;
