...
#endif
  for (size_t idx = 0; idx < n_rows; idx++) {
    int64_t pred_idx = hydraulis::impl::get_index(idx, ndims, stride_pred, c_shape);
    int64_t label_idx = hydraulis::impl::get_index(idx, ndims, stride_label, c_shape);
    int64_t loss_idx = hydraulis::impl::get_index(idx, ndims, stride_loss, c_shape);
