...
#include "hydraulis/utils/task_queue.h"
...
static int32_t get_shape_mismatch_flag() {
  auto& graph = hydraulis::graph::Graph::GetGraph(hydraulis::graph::Graph::cur_graph_ctx());
  if (graph.type() == hydraulis::graph::GraphType::EXECUTABLE) {
    return dynamic_cast<hydraulis::graph::ExecutableGraph&>(graph).shape_mismatch_flag();
  }
  return 0;
}
...
: CommunicationGroupDef(world_ranks, stream) {
  ...
  {
    hydraulis::cuda::CUDADeviceGuard guard(_stream.device_index());
    {
