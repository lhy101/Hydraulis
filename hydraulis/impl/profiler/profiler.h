...
#include "hydraulis/utils/optional.h"
#include <stack>
...
struct OpProfilerInfo {
  hydraulis::graph::OpType type;
  hydraulis::graph::OpName name;
...
class Profile {
 public:
  ...
  void push(hydraulis::graph::OpType type, hydraulis::graph::OpName name,
            HTShapeList inputs_shape, double cost_time) {
    _op_record.push_back({type, name, inputs_shape, cost_time});
  }

  void push(hydraulis::graph::Operator& op) {
    HT_VALUE_ERROR_IF(!_enabled) << "The Profiler is not enabled";
    _ops.push_back(op);
  }

  void push(hydraulis::graph::OpType type, double total_time) {
    HT_VALUE_ERROR_IF(!_enabled) << "The Profiler is not enabled";
  ...
  void sync_op() {
    if (!enabled())
      return;
    if (!_ops.empty()) {
      std::unordered_set<hydraulis::Stream> _sync_stream;
      for (auto& op : _ops) {
      ...
      for (auto& op : _ops) {
        HTShapeList inputs_shape;
	      hydraulis::graph::Operator::for_each_input_tensor(op, [&](const hydraulis::graph::Tensor& input) {
  ...
  std::vector<std::pair<hydraulis::graph::OpType, std::pair<double, std::pair<double, int>>>>
  get_optype_view() {
    sync_op();
    std::vector<std::pair<hydraulis::graph::OpType, std::pair<double, std::pair<double, int>>>> single_optype_total_time;
    std::map<hydraulis::graph::OpType, std::pair<double, int>> single_optype_total_time_unordered;
    ...
    std::sort(single_optype_total_time.begin(), single_optype_total_time.end(),
      [&](std::pair<hydraulis::graph::OpType, std::pair<double, std::pair<double, int>>> x, std::pair<hydraulis::graph::OpType, std::pair<double, std::pair<double, int>>> y) {
                return x.second.second.first > y.second.second.first; });
    return single_optype_total_time;
  }

  std::vector<std::pair<std::pair<hydraulis::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>>>
  get_optype_with_inputs_view() {
    sync_op();

    std::vector<std::pair<std::pair<hydraulis::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>>> single_optype_with_inputs_total_time;
    std::map<std::pair<hydraulis::graph::OpType, HTShapeList>, std::pair< double, int>> single_optype_with_inputs_total_time_unordered;
    for (auto& record: _op_record) {
    ...
    std::sort(single_optype_with_inputs_total_time.begin(), single_optype_with_inputs_total_time.end(),
      [&](std::pair<std::pair<hydraulis::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>> x, std::pair<std::pair<hydraulis::graph::OpType, HTShapeList>, std::pair<double, std::pair<double, int>>> y) {
            return x.second.second.first > y.second.second.first; });
    return single_optype_with_inputs_total_time;
  }

  std::vector<std::pair<hydraulis::graph::OpType, double>> get_graph_view() {
 ...
 protected:
  ...
  std::vector<hydraulis::graph::Operator> _ops;
